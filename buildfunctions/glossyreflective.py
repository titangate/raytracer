...
class BuildFunction(BuildFunctionBase):
    ...
    def build_function(cls, world, viewmode):
        ...
        mirror_mat = GlossyReflective(0.0, numpy.array((1.,1.,1.)), 500, sampler, kf=1.0)  # white
        mirror_mat_alt = GlossyReflective(0.0, numpy.array((.8,.3,.6)), 10, sampler, kf=1.0)  # white

        occluder = AmbientLight(numpy.array((1.,1.,1.)), .2)
        world.ambient_color = occluder

        sphere1 = Sphere(center=numpy.array((0,1,0)), radius=1., material=mirror_mat_alt)
        ...
        sphere3 = Sphere(center=numpy.array((3.5,1.5,2.5)), radius=2., material=mirror_mat)
        world.objects.append(sphere3)

        sphere4 = Sphere(center=numpy.array((2.5,0.5,-1.5)), radius=.7, material=mirror_mat_alt)
